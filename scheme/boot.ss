;;; boots up essential intepreter stuff, including module system
...
;;; optimized map/for-each for single list
...
(define (require-lowlevel sym environment)
  (define (resolve-name sym)
	(let ([val (cond
				[(string? sym) sym]
				[(symbol? sym) (symbol->string sym)]
				[else
				 (error 'require "Only string and symbol are allowed")])]
		  [sep (if (defined? '*folder-separator*) *folder-separator* "/")]
		  [ext ".ss"])
	  (let loop ([paths *load-path*])
		(if (null? paths)
			#f
			(let* ([folder (car paths)]
				   [full   (string-append folder sep val ext)])
			  (if (file-exists? full)
				  full
				  (loop (cdr paths)) ))))))

  ;; check if is already loaded
  (if (assoc sym *loaded-modules*)
	#t
	(let ([path (resolve-name sym)])
	  (if path
		(begin
		  ;; remember it first
		  (set! *loaded-modules*
				(cons (list sym path) *loaded-modules*))
		  ;; now load it
		  (eval (list load path) environment))
		(error 'require "Unable to find" sym "in *load-path* paths")))))

(define-macro (require symbol)
  `(require-lowlevel ,symbol (current-environment)))
